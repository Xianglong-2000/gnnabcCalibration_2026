...
def model_iter(parameters:dict): 
    ...
    go through things in order: pkl -> csv -> tr_csv -> graph
    """
...
def load_graph_data(graph_ID:str, node_features_file:str, neighbours_edge_file:str, include_columns:list):
    ...
    y1 = (math.log(y1_original) - gamma_mean) / gamma_std   ### standard normalization
    y2 = (math.log(y2_original) - reg_param_mean) / reg_param_std
    ...
    parents_edge_df = s1_trcsv2g_simdata.generate_parent_df(node_features_df)
    edge_features_df = pd.concat([parents_edge_df, neighbours_edge_df])

    for col in node_features_df.columns:
        node_features_df[col] = node_features_df[col].map(s1_trcsv2g_simdata.smart_cast)

    for col in edge_features_df.columns:
        edge_features_df[col] = edge_features_df[col].map(s1_trcsv2g_simdata.smart_cast)
    ...
    x = torch.tensor([s1_trcsv2g_simdata.flatten_node_attributes(G.nodes[node]) for node in G.nodes()], dtype=torch.float)
...
if __name__ == '__main__':
    ...
    final_path = parent_dir + "S1_test_1122.pkl"
    print(final_path)  
    ...
    #gamma = math.log(250)  ### for a simple test only
    #reg_param = math.log(30)  ### for a simple test only
