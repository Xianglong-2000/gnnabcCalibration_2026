...
def gnn_prediction(input_data) -> dict:
    ...
    ##print(output)
    p1 = round(float(output.tolist()[0][0]*gamma_std+gamma_mean),5)  ### de-normalized
    p2 = round(float(output.tolist()[0][1]*reg_param_std+reg_param_mean),5)  ### de-normalized
...
if __name__ == '__main__':
    ...
    n_cores = 2
    population_size = 50  ### set it to 4 for a simple test
    ...
    ##min_epsilon = 0.05  ### set it to 0.5 for a simple test
    ##max_populations = 4  ### set it to 3 for a simple test
