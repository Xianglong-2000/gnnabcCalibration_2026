...
import tr_csv2g_data_gatlstmlearner
...
def model(parameters:dict): 
    ...
    # csv to pre_tr_csv
    print("csv to pre_tr csv started")
    ...
    print("csv to pre_tr csv completed")

    # pre_tr_csv to tr_csv
    print("pre_tr csv to tr csv started")
    input_dir = output_dir
    output_dir = Path(f"D:/Projects/GNN Research/Data Files/_simulator_tr_csv_splitted_data_abc/{date_simulations}/gamma={gamma}_reg_param={reg_param}")
    os.makedirs(output_dir, exist_ok=True)
    ...
    result = subprocess.run(
        ...
        ##"D:/Projects/Conda Environments/gnn_cell_growth_env/python.exe",
         "-m", "Trackrefiner.cli",
    ...
    # tr_csv to graph
    # adapt this part to our s4 strategy
    ...
    g_data_path = os.path.join(folder_path, "CVIS_S4.pkl")
...
def load_graph_data(graph_ID:str, node_features_file:str, neighbours_edge_file:str, include_columns:list):########################
    ...
    y1 = math.log10(float(y1_original))
    y2 = math.log10(float(y2_original))
    ...
    parents_edge_df = tr_csv2g_data_gatlstmlearner.generate_parent_df(node_features_df)
    edge_features_df = pd.concat([parents_edge_df, neighbours_edge_df])

    for col in node_features_df.columns:
        node_features_df[col] = node_features_df[col].map(tr_csv2g_data_gatlstmlearner.smart_cast)

    for col in edge_features_df.columns:
        edge_features_df[col] = edge_features_df[col].map(tr_csv2g_data_gatlstmlearner.smart_cast)
...
if __name__ == '__main__':
    ...
    #gamma = math.log10(442.01081)  ### for a simple test only
    #reg_param = math.log10(1.08707)  ### for a simple test only
