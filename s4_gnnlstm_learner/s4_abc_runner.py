...
sys.path.append('D:/Projects/GNN Research/Code Files/my_code_files/s4')
...
def model_test(parameters:dict):
    ...
    path = "D:/Projects/GNN Research/Code Files/my_code_files/s4/abcsmc_iter_gatlstmlearner.py"
    envr = sys.executable
    time_limit = 1500
    try:
    ...
    pkl_file_path = f"D:/Projects/GNN Research/Data Files/_simulator_graph_splitted_data_abc/2025-09-25/{graph_id}"
    with open(pkl_file_path + "/CVIS_S4.pkl", "rb") as file:
        seq_dict = pk.load(file)  ### now the pickle is loaded
    ...
    e1 = [float(param[0]) for param in preds.tolist()][0]
    e2 = [float(param[1]) for param in preds.tolist()][0]
    e3 = [float(param[2]) for param in preds.tolist()][0]
    e4 = [float(param[3]) for param in preds.tolist()][0]
    e5 = [float(param[4]) for param in preds.tolist()][0]
    e6 = [float(param[5]) for param in preds.tolist()][0]
    e1 = (e1 - min_ss1)/(max_ss1 - min_ss1)
    e2 = (e2 - min_ss2)/(max_ss2 - min_ss2)
    e3 = (e1 - min_ss3)/(max_ss3 - min_ss3)
    e4 = (e1 - min_ss4)/(max_ss4 - min_ss4)
    e5 = (e1 - min_ss5)/(max_ss5 - min_ss5)
    e6 = (e1 - min_ss6)/(max_ss6 - min_ss6)
...
if __name__ == '__main__':
    ...
    # get mean and std from 1000 simulations for GNN
    ...
    # get min and max from 1000 simulations for GNN
    parent_dir = f"D:/Projects/GNN Research/Data Files/_sim_mean_std_values_gnn/{date_simulations}/"  
    final_path = parent_dir + "CVIS_S4_minmaxss.pkl"  
    print(final_path) 
    with open(final_path, "rb") as file: 
        values = pk.load(file) 
    ##print(values)  
    ...
    saved_model_path = "D:/Projects/GNN Research/Data Files/_model_data_old/CVIS_S4_minmaxtest.pth"
    ...
    ##parameters = {"gamma":math.log10(250), "reg_param":math.log10(35)}  ### for a simple test only
    ##result = model_test(parameters)  ### for a simple test only
    ##print(result)  ### for a simple test only

    # Step 0: load sim-exp g_data
    file_path = "D:/Projects/GNN Research/Data Files/_sim_graph_splitted_data_gnn/2025-08-24_s4"
    ...
    e1_list = [(float(param[0]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    e2_list = [(float(param[1]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    e3_list = [(float(param[2]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    e4_list = [(float(param[3]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    e5_list = [(float(param[4]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    e6_list = [(float(param[5]) - min_ss1)/(max_ss1 - min_ss1) for param in preds.tolist()]
    exp_summary_stats = {}
    ...
    abc = pyabc.ABCSMC(model_test, 
                       ...
                       sampler=pyabc.sampler.SingleCoreSampler()    ### will cause error if set it to MultipleCoreSampler() for some reason
                       )
    ...
    # Step x: run
    ##breakpoint()  ### to check completed generations
