...
sys.path.append('/work/x5bai/project/Code_Files/general/')
...
def model_runner(parameters:dict):
    ...
    replicates = 10
    sim_summary_stats = {"e1": [],"e2": [], "e3": [], "e4": [], "e5": [], "e6": []}
    ...
    for i in range(replicates):
        ...
        if i == 0:
            print(f"currently running: gamma={gamma} | alpha={reg_param}")
        with open(g_data_path, "rb") as file:
            data = pk.load(file)

        # gnn prediction as the ss
        result = gnn_prediction(data)
        sim_summary_stats["e1"] += [result["e1"]]
        sim_summary_stats["e2"] += [result["e2"]]
        sim_summary_stats["e3"] += [result["e3"]]
        sim_summary_stats["e4"] += [result["e4"]]
        sim_summary_stats["e5"] += [result["e5"]]
        sim_summary_stats["e6"] += [result["e6"]]

        # delete all files for the current replicate
        ...
        g_path = f"/work/x5bai/project/Data_Files/_simulator_graph_data_abc/{date_simulations}/abc_g_data_gamma={gamma}_reg_param={reg_param}"
        shutil.rmtree(pkl_path, ignore_errors=True)
        shutil.rmtree(csv_path, ignore_errors=True)
        shutil.rmtree(trcsv_path, ignore_errors=True)
        shutil.rmtree(g_path, ignore_errors=True)

    print("sim ss: ", sim_summary_stats)
...
if __name__ == '__main__':
    ...
    exp_summary_stats = {}
    exp_summary_stats["e1"] = np.asarray(e1_list)
    exp_summary_stats["e2"] = np.asarray(e2_list)
    exp_summary_stats["e3"] = np.asarray(e3_list)
    exp_summary_stats["e4"] = np.asarray(e4_list)
    exp_summary_stats["e5"] = np.asarray(e5_list)
    exp_summary_stats["e6"] = np.asarray(e6_list)
