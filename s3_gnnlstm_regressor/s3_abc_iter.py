...
import tr_csv2g_data_gatlearner
...
def model(parameters:dict): 
    ...
    try:
        ...
        try:
            ...
            simulate(moduleName, reg_param=reg_param, gamma=gamma, max_cells=max_cells, export_path=export_path)
            os.makedirs(f"D:/Projects/GNN Research/Data Files/_simulator_pkl_data_abc/sim_error_counter/error2_{gamma}_{reg_param}", exist_ok=True)
        else:
            os.makedirs(f"D:/Projects/GNN Research/Data Files/_simulator_pkl_data_abc/sim_error_counter/error1_{gamma}_{reg_param}", exist_ok=True)
    ...
    folder_path = f"D:/Projects/GNN Research/Data Files/_simulator_graph_data_abc/{date_simulations}/"
...
def load_graph_data(graph_ID:str, node_features_file:str, neighbours_edge_file:str, include_columns:list):########################
    ...
    y1 = math.log10(y1_original)
    y2 = math.log10(y2_original)
    ...
    parents_edge_df = tr_csv2g_data_gatlearner.generate_parent_df(node_features_df)
    edge_features_df = pd.concat([parents_edge_df, neighbours_edge_df])

    for col in node_features_df.columns:
        node_features_df[col] = node_features_df[col].map(tr_csv2g_data_gatlearner.smart_cast)

    for col in edge_features_df.columns:
        edge_features_df[col] = edge_features_df[col].map(tr_csv2g_data_gatlearner.smart_cast)
    ...
    x = torch.tensor([tr_csv2g_data_gatlearner.flatten_node_attributes(G.nodes[node]) for node in G.nodes()], dtype=torch.float)
