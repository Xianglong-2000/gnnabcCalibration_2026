...
if __name__ == '__main__':
    ...
    ##num_iter = 1000
    num_iter = 24*9  ### for feature check 2026/02/04
    dt_sim = "2026-02-04"
    ...
    for i in range(num_iter):
        if i < 10:  # the date here is always the date when the pickle files were generated
            parent_dir = f"/work/x5bai/project/Data_Files/_sim_pkl_data_gnn/{dt_sim}/iteration 00{i}/"  # pickle folder path
            new_dir = f"/work/x5bai/project/Data_Files/_sim_csv_data_gnn/{dt_sim}/iteration 00{i}"  # csv folder path
        elif (i >= 10) and (i <100):
            parent_dir = f"/work/x5bai/project/Data_Files/_sim_pkl_data_gnn/{dt_sim}/iteration 0{i}/"  # pickle folder path
            new_dir = f"/work/x5bai/project/Data_Files/_sim_csv_data_gnn/{dt_sim}/iteration 0{i}"  # csv folder path
        elif (i >= 100) and (i <1000):
            parent_dir = f"/work/x5bai/project/Data_Files/_sim_pkl_data_gnn/{dt_sim}/iteration {i}/"  # pickle folder path
            new_dir = f"/work/x5bai/project/Data_Files/_sim_csv_data_gnn/{dt_sim}/iteration {i}"  # csv folder path
        else:
