...
sys.path.append('D:/Projects/GNN Research/Code Files/my_code_files/general/FOSBE2024')
import kl_divergence

def model_runner(parameters:dict):

    # subprocess the iteration to genereate and save ss
    path = "D:/Projects/GNN Research/Code Files/my_code_files/s5/abcsmc_iter_traditionalss.py"
    envr = sys.executable
    time_limit = 3000

    sim_summary_stats = {"Aspect ratio": [],
                         "Order parameter": [],
                         "Convexity": [],
                         "Density": [],
                         "Agreement with exponential growth": [],
    ...
    for i in range(replicates):
        ...
        with open(pkl_path, "rb") as file:
            result = pk.load(file)
        print(f"the {i}th replicate ss: ", result)  ### shouldn't be the same between replicates

        sim_summary_stats["Aspect ratio"] += [result["Aspect ratio"]]
        sim_summary_stats["Order parameter"] += [result["Order parameter"]]
        sim_summary_stats["Convexity"] += [result["Convexity"]]
        sim_summary_stats["Density"] += [result["Density"]]
        sim_summary_stats['Agreement with exponential growth'] += [result['Agreement with exponential growth']]
        sim_summary_stats['Normalized growth rate'] += [result['Normalized growth rate']]

        # delete the current replicate
        shutil.rmtree(folder_path, ignore_errors=True)

    return sim_summary_stats

   
def distance_calculation(sim_stats, exp_stats):  ### KL Divergence
    distance_list = []
    for x in sim_stats.keys():
        diff = kl_divergence.get_kl_divergence(exp_stats[x], sim_stats[x])
...
if __name__ == '__main__':
    ...
    ss_df = pd.read_csv("D:/Projects/GNN Research/Data Files/_check/real-exp_data/OneDrive_1_2025-10-29/exp_summary_stat.csv")
    ...
    exp_summary_stats = {}
    exp_summary_stats["Aspect ratio"] = np.asarray(Aspect_ratio_list)
    exp_summary_stats['Convexity'] = np.asarray(Convexity_list)
    exp_summary_stats["Order parameter"] = np.asarray(Order_parameter_list)
    exp_summary_stats["Density"] = np.asarray(Density_list)
    exp_summary_stats["Agreement with exponential growth"] = np.asarray(Agreement_with_exponential_growth_list)
    exp_summary_stats["Normalized growth rate"] = np.asarray(Normalized_growth_rate_list)
